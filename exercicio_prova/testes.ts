...
class TestesBancoLimite {
    ...
    private criarClienteNoBanco(nome: string, cpf: string): Cliente {
        const idTemporario = 0;
        const cliente = new Cliente(idTemporario, nome, cpf, new Date(1990, 0, 1), []);
    ...
    private criarContaNoBanco(numero: string, saldo: number, limite: number): Conta {
        const idTemporario = 0;
        const idConstrutorTemporario = 0;
        const clienteTemporario = new Cliente(0, "temporario", "000", new Date(), []);

        const conta = new Conta(idTemporario, numero, clienteTemporario, new Date(), saldo, limite, idConstrutorTemporario);
    ...
    private teste1Deposito(): void {
        this.rodarTeste(
            ...
            () => {
                ...
                const idConstrutorTemporario = 0;
                const clienteTemporario = new Cliente(0, "temporario", "000", new Date(), []);
                const numeroTemporario = "0";
                const saldoTemporario = 0;
                const limiteTemporario = 0;
                const conta = new Conta(idTemporario, numeroTemporario, clienteTemporario, new Date(), saldoTemporario, limiteTemporario, idConstrutorTemporario);
    ...
    private teste2SaqueComLimite(): void {
        this.rodarTeste(
            ...
            () => {
                ...
                const clienteTemporario = new Cliente(0, "temporario", "000", new Date(), []);
                const numeroTemporario = "0";
                // saldo 100, limite 100
                const conta = new Conta(idTemporario, numeroTemporario, clienteTemporario, new Date(), 100, 100, idConstrutorTemporario);
                ...
                if (!maisRecente || !segunda || !terceira){
                    throw new Error("Operação não encontrada");
                }
    ...
    private teste3TransferenciaComLimite(): void {
        this.rodarTeste(
            ...
            () => {
                ...
                const saldoTemporario = 0;
                const limiteTemporario = 0; 
                const origem = new Conta(idTemporario, "100", clienteTemporario, new Date(), 100, 200, idConstrutorTemporario);
                const destino = new Conta(idTemporario, "200", clienteTemporario, new Date(), 50, 0, idConstrutorTemporario);
                ...
                if (!opFalha) {
                    throw new Error("Operção não encontrada");
                }
